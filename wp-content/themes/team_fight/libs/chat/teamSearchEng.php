	...
	public function searchTeam($game) {
		...
		if ($game == 'cs_go') {
			...
			$sql = "SELECT `".$this->user_id_field."` 
				...
				AND NOT `pubg_st` = 'in_chat'
				AND `last_active` > NOW() - INTERVAL 6 SECOND;"; // Leo Added
...
// 			var_dump($mateRow);
			if (mysqli_num_rows($result) > 0) {
				...
				if ($tableRow[$this->user_team_num_field] == 2) {
					if (count($team_mate) > 1) {
						$last_mate = count($team_mate) - 1;
						$mate_num = rand(0,$last_mate);
					} elseif (count($team_mate)==1) {
						$mate_num = 0; 
					}
			...
			$min_mmr = $tableRow[$game]-400;
			$max_mmr = $tableRow[$game]+400;
			$team_num = $this->db->query("SELECT `".$this->user_team_num_field."` FROM `".$this->user_table."` WHERE `".$this->user_id_field."` = '".$this->steamid."'");
			$tableRow = mysqli_fetch_assoc( $team_num );
			$sql = "SELECT `".$this->user_id_field."` 
				FROM `".$this->user_table."`
				WHERE `dota_st` = 'yes'
				AND `".$this->user_team_num_field."` = '".$tableRow[$this->user_team_num_field]."'
				AND (`dota` BETWEEN ".$min_mmr." AND ".$max_mmr.")
				AND NOT ".$this->user_id_field." = '".$this->steamid."'
			...
			/*$mode = $this->db->query("SELECT `".$game."` FROM `".$this->user_table."` WHERE `".$this->user_id_field."` = '".$this->steamid."'");
			$mode_play = mysqli_fetch_assoc( $mode );*/
			$user = $this->db->query("SELECT `*` FROM `".$this->user_table."` WHERE `".$this->user_id_field."` = '".$this->steamid."'");
			$tableRow = mysqli_fetch_assoc( $user );


			$sql = "SELECT `".$this->user_id_field."` 
				FROM `".$this->user_table."`
				WHERE `pubg_st` = 'yes'
				AND `pubg_num` = '".$tableRow['pubg_num']."'
				AND `pubg` = '".$tableRow['pubg']."'
				AND NOT ".$this->user_id_field." = '".$this->steamid."'
				AND NOT `cs_go_st` = 'in_chat'
				AND NOT `dota_st` = 'in_chat'
				AND `last_active` > NOW() - INTERVAL 6 SECOND;"; // Leo Added
			...
			if (mysqli_num_rows($result) > 0) {
				...
				if ($tableRow['pubg_num'] == 2) {
					...
					$this->db->saveUserData($team_mate[$mate_num], "pubg_st", "in_chat");
					$this->db->saveUserData($this->steamid, "pubg_st", "in_chat");
					return $team_mate[$mate_num];
				} elseif ($tableRow['pubg_num'] == 4) {
					if (count($team_mate) >= 3) {
						$last_mate = count($team_mate) - 1;
						$mate_num_1 = rand(0,$last_mate);
						do {   
						    $mate_num_2 = rand(0,$last_mate);
						...
						$this->db->saveUserData($this->steamid, "pubg_st", "in_chat");
						$this->db->saveUserData($team_mate[$mate_num_1], "pubg_st", "in_chat");
						$this->db->saveUserData($team_mate[$mate_num_2], "pubg_st", "in_chat");
						$this->db->saveUserData($team_mate[$mate_num_3], "pubg_st", "in_chat");
						return $team_mates;
					} elseif (count($team_mate)==1) {
						return 0;
					} 
